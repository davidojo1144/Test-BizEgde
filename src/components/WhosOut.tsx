...
export function WhosOut() {
  const { whosOut } = useAppSelector((state) => state.dashboard)
        ...
        <div className="flex gap-4">
          <Button variant="outline" size="sm" className="bg-gray-100 flex items-center gap-2">
            ...
            <Badge variant="destructive" className="rounded-full w-4 h-4 p-0 flex items-center justify-center text-xs">
              {whosOut.filter((w) => w.status === "upcoming").length}
            </Badge>
          </Button>
          <Button variant="ghost" size="sm" className="text-gray-500 flex items-center gap-2">
            Ongoing
            <Badge variant="destructive" className="rounded-full w-4 h-4 p-0 flex items-center justify-center text-xs">
              {whosOut.filter((w) => w.status === "ongoing").length}
            </Badge>
          </Button>
        </div>
            ...
            <div key={request.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <div className="text-right">
                <Badge className="bg-orange-100 text-orange-800 mb-1">
                  {request.type === "sick" ? "Sick Leave" : "Vacation"}
                </Badge>
